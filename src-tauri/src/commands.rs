...
fn validate_master_password(password: &str) -> crate::error::Result<()> {
    ...
    if !(12..=1024).contains(&length) {
        return Err(VaultError::Validation(
            "master password must contain 12 to 1024 characters".into(),
        ));
    }
    Ok(())
}
...
fn snapshot(session: &VaultSession) -> SessionSnapshot {
    ...
    SessionSnapshot {
        entries,
        settings: session.data.settings.clone(),
    }
}
...
fn unlock_imported_envelope(
    state: &AppState,
    envelope: crate::model::VaultEnvelope,
    master_password: &str,
) -> crate::error::Result<SessionSnapshot> {
    ...
    *state
        .session
        .lock()
        .map_err(|_| VaultError::Storage("session lock poisoned".into()))? = Some(session);
    Ok(response)
}
...
pub fn create_vault(
    ...
    let result = (|| {
        ...
        let session = VaultSession {
            data: crate::model::VaultData::new(),
            key,
            kdf,
        };
...
pub fn lock_vault(state: State<'_, AppState>) -> CommandResult<()> {
    let mut guard = state.session.lock().map_err(|_| {
        CommandError::from(VaultError::Storage("session lock poisoned".into()))
    })?;
    *guard = None;
    Ok(())
}
...
pub fn get_entry(state: State<'_, AppState>, id: Uuid) -> CommandResult<VaultEntry> {
    ...
    session
        ...
        .cloned()
        .ok_or_else(|| CommandError::from(VaultError::Validation("entry was not found".into())))
}
...
pub fn upsert_entry(
    ...
    command((|| {
        ...
        let mut guard = state
            .session
            .lock()
            .map_err(|_| VaultError::Storage("session lock poisoned".into()))?;
        let session = guard.as_mut().ok_or(VaultError::Locked)?;
        let now = Utc::now();
        match input
            .id
            .and_then(|id| session.data.entries.iter().position(|entry| entry.id == id))
        {
            Some(index) => {
...
pub fn delete_entry(state: State<'_, AppState>, id: Uuid) -> CommandResult<SessionSnapshot> {
    command((|| {
        ...
        if before == session.data.entries.len() {
            return Err(VaultError::Validation("entry was not found".into()));
        }
...
pub fn export_vault(state: State<'_, AppState>, destination: String) -> CommandResult<()> {
    command((|| {
        ...
        if path.extension().and_then(|value| value.to_str()) != Some("vaultora") {
            return Err(VaultError::Validation(
                "export filename must use the .vaultora extension".into(),
            ));
        }
...
#[tauri::command]
pub fn export_vault_base64(state: State<'_, AppState>) -> CommandResult<String> {
    command((|| {
        let guard = state
            .session
            .lock()
        ...
        let bytes = state.storage.export_bytes()?;
...
pub fn import_vault(
    ...
    let result = (|| {
        ...
        unlock_imported_envelope(&state, envelope, &master_password)
    })();
    master_password.zeroize();
    command(result)
}
...
pub fn import_vault_base64(
    ...
    let result = (|| {
        let mut bytes = BASE64
            ...
            .map_err(|_| VaultError::Validation("backup payload is not valid base64".into()))?;
        let envelope_result = state.storage.import_bytes(&bytes);
        bytes.zeroize();
...
pub fn change_master_password(
    ...
    let result = (|| {
        ...
        *state
            .session
            .lock()
            .map_err(|_| VaultError::Storage("session lock poisoned".into()))? = Some(replacement);
        Ok(())
    })();
