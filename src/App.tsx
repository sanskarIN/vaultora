...
export default function App() {
  ...
  useAutoLock(Boolean(snapshot), snapshot?.settings.auto_lock_minutes ?? 5, lock);
  ...
  async function selectEntry(entry: EntrySummary) {
    ...
    try {
      setSelectedEntry(await api.getEntry(entry.id));
    } catch {
      setSelectedEntry(null);
    }
  }
  ...
  async function saveEntry(input: Parameters<typeof api.upsertEntry>[0]) {
    ...
    setSnapshot(next);
    if (input.id) {
      await selectEntry(next.entries.find((entry) => entry.id === input.id) ?? next.entries[0]!);
    }
  }
  ...
  if (!snapshot) {
    return (
      <AuthGate
        vaultExists={exists}
        onVaultCreated={() => setExists(true)}
        onUnlocked={setSnapshot}
      />
    );
  }
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Primary navigation">
          <NavButton active={screen === "vault"} onClick={() => setScreen("vault")} icon="▣">
            Vault
          </NavButton>
          <NavButton
            active={screen === "generator"}
          ...
          <NavButton active={screen === "about"} onClick={() => setScreen("about")} icon="ⓘ">
            About
          </NavButton>
        </nav>
        <div className="sidebar-footer">
          <button className="button ghost full" onClick={lock}>
            {strings.lock}
          </button>
      ...
      <main className="workspace">
        {screen === "vault" && (
          <>
            <header className="workspace-header">
              <div>
                <p className="eyebrow">Encrypted locally</p>
                <h1>Your vault</h1>
              </div>
              <button
            ...
            <div className="toolbar">
              ...
              <select
                ...
                value={kind}
                onChange={(event) => setKind(event.target.value as EntryKind | "all")}
              >
                <option value="all">All types</option>
                <option value="login">Logins</option>
                <option value="secure_note">Secure notes</option>
              ...
              <section className="entry-list" aria-label="Vault entries">
                {visibleEntries.length === 0 ? (
                  <div className="empty-state">
                    <span>◈</span>
                    <h2>{snapshot.entries.length ? strings.noMatches : strings.noEntries}</h2>
                  </div>
                ) : (
                  visibleEntries.map((entry) => (
                    ...
                    >
                      <div className="entry-icon" aria-hidden="true">
                        {entry.kind === "login" ? "↗" : entry.kind === "secure_note" ? "≡" : "◎"}
                      </div>
                      <div className="entry-main">
                        <strong>{entry.name}</strong>
                        <span>{entry.username || entry.url || kindLabel(entry.kind)}</span>
                      </div>
                      {entry.favorite && (
              ...
              <section className="detail-region">
                {selectedEntry ? (
                  ...
                  <div className="detail-placeholder">
                    ...
                    <p>Protected values are only loaded when you open an entry.</p>
                  </div>
                )}
              </section>
            </div>
        ...
        {screen === "settings" && (
          <SettingsPanel settings={snapshot.settings} onUpdated={setSnapshot} />
        )}
